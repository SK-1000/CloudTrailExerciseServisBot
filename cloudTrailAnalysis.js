...
//this sets up the email transport service using Gmail. Authentication credentials (email and password) are retrieved from environment variables.
...
const rateRisk = (event, isIpValid, failedLoginAttempts, alerts) => {
  ...
  const isManualDeletion =
    /Delete/i.test(event.eventName) &&
    (event.userIdentity?.type === "IAMUser" ||
      event.userIdentity?.type === "Root");

  if (isManualDeletion) {
    reason.push(
      "HIGH RISK: Manual Deletion may have occurred; investigation required,",
    );
    alerts.push(
      `HIGH RISK: Manual deletion detected by ${event.userIdentity.arn} on ${event.eventName}. Event ID: ${event.eventID}.`,
    );
  }

  // Check for IAM policy creation - forbidden
  const isIamPolicyCreation =
  ...
  if (isIamPolicyCreation) {
    ...
    alerts.push(
      `HIGH RISK: User (${userName}) created a new IAM policy. Event ID: ${event.eventID}.`,
    );
    // riskRating = 'High';
    reason.push(
      `HIGH RISK: User has illegally created a new IAM policy: ${userName},`,
    );
  }
  ...
  const isIamPolicyChange =
    event.eventSource === "iam.amazonaws.com" &&
    iamChangeActions.includes(event.eventName);
  ...
  if (!isIpValid) {
    ...
    alerts.push(
      `MEDIUM RISK: Event ID ${event.eventID} from outside allowed IP ranges: ${event.sourceIPAddress}.`,
    );
  }
  ...
  if (!usEuRegions.includes(event.awsRegion)) {
    ...
    setRisk("Medium", `MEDIUM RISK: Event from an unsupported AWS region: ${event.awsRegion},`);
    alerts.push(
      `MEDIUM RISK: Event ID ${event.eventID} detected in unsupported AWS region: ${event.awsRegion}.`,
    );
  }

  if (!approvedServices.includes(event.eventSource)) {
    // riskRating = 'Medium';
    setRisk("Medium", "MEDIUM RISK: Event using unapproved service");
    alerts.push(
      `MEDIUM RISK: Event ID ${event.eventID} is using a service outside approved services: ${event.eventSource}.`,
    );
  }
  ...
  if (isIamPolicyChange) {
    const userName = event.userIdentity?.userName || "Unknown User";
    alerts.push(
      `HIGH RISK: User (${userName}) changed IAM policy. Event ID: ${event.eventID}.`,
    );
    
    setRisk("High", `HIGH RISK: User has made changes to IAM policies: ${userName},`);
  }

  // Check for high-risk operations on production resources
  const isProductionTaggedEC2 =
    event.eventSource === "ec2.amazonaws.com" &&
    (event.requestParameters?.tagSet?.some(
      (tag) => tag.Key === "Environment" && tag.Value === "production",
    ) ||
      event.requestParameters?.instancesSet?.items?.some(
        (item) =>
  ...
  const isProductionTagBucketS3 =
    event.eventSource === "s3.amazonaws.com" &&
    event.requestParameters?.bucketName?.includes("prod");

  if (
    (isProductionTagBucketS3 || isProductionTaggedEC2) &&
    isHighRiskOperation
  ) {
    
    setRisk("High", "HIGH RISK: Deletion or modification of production resource; review required,");
    alerts.push(
      `HIGH RISK: Deletion/modification in production resource detected. Event ID: ${event.eventID}.`,
    );
  }

  // Compliance checks
  const isS3WithEncryption =
    event.eventSource === "s3.amazonaws.com" &&
    event.requestParameters?.ServerSideEncryption === "aws:kms";
  const isRDSWithEncryption =
    event.eventSource === "rds.amazonaws.com" &&
    event.requestParameters?.StorageEncrypted;
  const isEncryptedFileSystem =
    event.eventSource === "elasticfilesystem.amazonaws.com" &&
    event.requestParameters?.Encrypted;

  if (isS3WithEncryption || isRDSWithEncryption || isEncryptedFileSystem) {
    
    setRisk(
      "High",
      "HIGH RISK: Event does not meet compliance requirements for encryption,",
    );
    alerts.push(
      `HIGH RISK: Event ID ${event.eventID} does not meet compliance requirements for encryption.`,
    );
  }
  ...
  if (event.userIdentity?.type === "Root") {
    ...
    alerts.push(
      `HIGH RISK: Event ID ${event.eventID} detected from Root user: ${event.userIdentity.arn}.`,
    );
  }
  ...
  if (isFailedLogin) {
    ...
    if (failedLoginAttempts.length >= 5) {
      // Threshold for multiple attempts
      setRisk("High", "High RISK: Multiple failed login attempts require review,");

      alerts.push(
        `High RISK: User ${event.userIdentity.arn} has multiple failed login attempts.`,
      );
    }
  }
      ...
      return new Promise((resolve, reject) => {
        fs.readFile(filePath, "utf8", (err, data) => {
          ...
          eventsArray.forEach((event) => {
            ...
            const { riskRating, reason } = rateRisk(
              event,
              isIpValid,
              failedLoginAttempts,
              alerts,
